...
fragColor = waterColor;
    ...
    `)),P.include(d.multipassTerrainTest,e),e.output){case i.ShaderOutput.Alpha:P.include(r.SliceDraw,e),D.uniforms.add(A),D.code.add(y.glsl`
        void main() {
          discardBySlice(vpos);
      ...
      `);break;case i.ShaderOutput.Color:P.include(s.MainLighting),P.include(n.EvaluateAmbientLighting,{pbrMode:v.PBRMode.Disabled,lightingSphericalHarmonicsOrder:2}),P.include(p.WaterDistortion),P.include(r.SliceDraw,e),P.include(c.ReadShadowMapDraw,e),P.include(u.Water,e),D.uniforms.add(A,new f.FloatPassUniform("timeElapsed",(e=>e.timeElapsed)),O.uniforms.get("view"),O.uniforms.get("localOrigin")),w.addCameraPosition(D,e),D.include(h.ColorConversion),s.addMainLightDirection(D),s.addMainLightIntensity(D),D.code.add(y.glsl`
      void main() {
        discardBySlice(vpos);
        ${e.multipassEnabled?"terrainDepthTest(depth);":""}
    ...
    `);break;case i.ShaderOutput.Normal:P.include(g.NormalUtils,e),P.include(p.WaterDistortion,e),P.include(r.SliceDraw,e),P.varyings.add("vpos","vec3"),P.varyings.add("vuv","vec2"),O.uniforms.add(A),O.code.add(y.glsl`
        void main(void) {
          if (waterColor.a < ${y.glsl.float(m.symbolAlphaCutoff)}) {
            // Discard this vertex
            gl_Position = vec4(1e38, 1e38, 1e38, 1.0);
...
}`);break;case i.ShaderOutput.Highlight:P.include(t.OutputHighlight,e),P.varyings.add("vpos","vec3"),O.uniforms.add(A),O.code.add(y.glsl`
      void main(void) {
        if (waterColor.a < ${y.glsl.float(m.symbolAlphaCutoff)}) {
          // Discard this vertex
          gl_Position = vec4(1e38, 1e38, 1e38, 1.0);
...
discardBySlice(vpos);
outputHighlight();
}`);break;case i.ShaderOutput.ObjectAndLayerIdColor:P.include(l.ObjectAndLayerIdColor,e),P.varyings.add("vpos","vec3"),O.uniforms.add(A),O.code.add(y.glsl`
      void main(void) {
        if (waterColor.a < ${y.glsl.float(m.symbolAlphaCutoff)}) {
          // Discard this vertex
          gl_Position = vec4(1e38, 1e38, 1e38, 1.0);
